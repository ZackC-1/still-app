...
// Regression guard for the popup width-collapse bug (fixed 2026-07-24).
//
// A browser-action popup has NO predefined viewport: the browser derives the popup window's width
// FROM the rendered content. A viewport unit (`100vw`) therefore resolves to ~0 during that
// measurement pass, so `inline-size: min(380px, 100vw)` collapsed the popup to a one-character-wide
// sliver. The only reliable popup width is a hard pixel value. This test fails if anyone reintroduces
// a viewport unit into the popup's sizing.

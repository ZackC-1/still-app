...
- [mirror-fixes-across-parallel-paths](mirror-fixes-across-parallel-paths.md) — the manual "check the sibling path" discipline this convention automates for rendered surfaces. Neither supersedes the other: that doc's fix is a shared code path; this one's is a CI-enforced contract for codebases that cannot share code.

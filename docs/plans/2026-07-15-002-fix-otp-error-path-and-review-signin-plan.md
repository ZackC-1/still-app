...
- **Review "send code" is a server preflight, and the drift fallback covers BOTH actions.** The port calls the function's `request` action; success renders the normal "code sent" state (no email exists — the reviewer has the code from the notes). Fallback discrimination: the function's not-configured refusal (404-shaped) or a 5xx/network failure falls back to the real OTP send; a 429 does NOT fall back — it renders the send wait state with the function's genuine retry-after (falling back on 429 would convert every throttled review request into a real email, burning the hourly SMTP budget and defeating the throttle). On verify, the same not-configured refusal falls through to the normal GoTrue `verifyOtp` — without this, the drift scenario's fallback email carries a code that could never verify (the client would keep routing verify to the refusing function), un-doing the fallback's whole point. The refusal shapes are a stable, tested client-routing contract: 404-shaped = not configured / email mismatch (fall back), 401 = wrong fixed code (invalid-code), 429 = rate-limited (wait state). Rationale: a silent client no-op plus missing server secrets is an unrecoverable reviewer dead end (guaranteed re-rejection); the preflight converts config drift into a degraded-but-working path for the account owner and gives the request side real rate limiting and observability.
